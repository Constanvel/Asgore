...
assert.equal(run('game.x'), run('ARENA.x + 12'));
assert.equal(run('game.y'), run('ARENA.y + 12'));
...
run('resizeGame()');
assert.equal(run('ARENA.w'), 520);
assert.equal(run('ARENA.h'), 300);
...
run('game.hazards = []; spawnPlatforms(0); game.y = ARENA.y + ARENA.h - 12; game.grounded = true; keys.add(" "); updatePlayer(1 / 120)');
assert.ok(run('game.vy < 0 && game.y < ARENA.y + ARENA.h - 12'), 'platform mode dapat melompat');
...
// Regresi combo screenshot: emitter harus berada di luar koridor aman.
run('resetGame(); openMenu(); startAttack(); game.phase = 4; game.pattern = 5; game.wave = 4; game.spawnTime = 99; spawnAttackPattern()');
assert.ok(run('game.hazards.filter(h => h.type === "pinwheel").every(h => h.y < h.safeBand[0] - 8 || h.y > h.safeBand[1] + 8)'));
run('game.y = game.hazards.find(h => h.type === "wall").gapY + game.hazards.find(h => h.type === "wall").gap / 2; for (let i = 0; i < 180; i++) update(1/120)');
assert.ok(run('game.bullets.length > 0'), 'combo pusaran mengeluarkan peluru hidup');
assert.equal(run('game.hp'), 40, 'jalur aman tetap aman');
run('resetGame(); openMenu(); startAttack(); game.hazards = []; game.bullets = []; game.spawnTime = 99; spawnPlatforms(0)');
assert.equal(run('game.hazards.filter(h => h.type === "platform").length'), 4);
run('game.hazards.forEach(h => h.age = h.warn); game.hazards = game.hazards.filter(h => h.type === "platform"); game.x = game.hazards[0].x + game.hazards[0].w / 2; game.y = game.hazards[0].y - 7; game.grounded = true; keys.clear(); touchTarget = null');
const carriedX = run('game.x');
run('update(.02)');
assert.notEqual(run('game.x'), carriedX, 'pemain terbawa pijakan bergerak');
assert.equal(run('game.grounded'), true);
